...
Теги — из словаря \`99_Система/Теги.md\`, ≥1 на канон. Нет подходящего — добавь новый прямо в словаре (явно, не на лету) и сразу ставь: добавить тег по потребности может ЛЮБОЙ автор, ждать никого не нужно. Index КУРИРУЕТ словарь (дедуп близких тегов, связность границ) — поэтому добавляй осознанно, не плоди дубли; роли комплементарны (ты добавляешь, Index поддерживает целостность).
...
- **блок \`## Связи\`** — отдельная связь к смежной заметке, которой в тексте нет, но она по смыслу рядом. **Позиция — В КОНЦЕ заметки, после тела.** Каждая строка: \`- [[X]] — чем связана\` (смежное / развивает / противоречит / применяет). Пояснение обязательно — одной фразой назови суть связи; это заодно не даёт линковать вслепую. Теги — во frontmatter, не в этом блоке.
...
Из frontmatter меняешь ТОЛЬКО \`status\` (по шкале типа из таблицы «Структура хранилища» выше). \`type\`, структуру тегов и чужую оперативку не трогаешь. **\`needs_review\` — флаг МЕХАНИКИ, не твой:** ставит его сама механика, снимает Индекс или человек — НЕ ты. Появился флаг после твоей правки — НЕ снимай его и НЕ пиши Индексу / не уведомляй его об этом: Индекс сам обходит флагнутые заметки фоном по каденции курации (≈6 часов). Просто продолжай работу — делать или сообщать ничего не нужно.
