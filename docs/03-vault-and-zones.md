...
On every edit the system stamps `last_edited_by` — who last changed the file. Every non-curator edit — the author's own included — is flagged for review (`needs_review`): the flag is the curator's proofreading queue, not a block, and it stays on the note until curation clears it. Only the curator roles (Index, Scriber, DreamWeaver) are exempt — their edits are sanctioned. Edits made around the system (in Obsidian, by hand) are recognized too — the note gets the editor's authorship and a review flag, and those edits aren't lost.
